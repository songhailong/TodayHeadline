...
open class BMPlayer: UIView {
    ...
    deinit {
        playerLayer?.pause()
        playerLayer?.prepareToDeinit()
        //NotificationCenter.default.removeObserver(self, name: NSNotification.Name.UIApplication.didChangeStatusBarOrientationNotification, object: nil)
    ...
    fileprivate func initUIData() {
       ...
       // er(self, selector: #selector(self.onOrientationChanged), name: NSNotification.Name.UIApplicationDidChangeStatusBarOrientation, object: nil)
       
        NotificationCenter.default.addObserver(self, selector: #selector(onOrientationChanged), name: UIApplication.didChangeStatusBarFrameNotification, object: nil)
...
extension BMPlayer: BMPlayerControlViewDelegate {
   ...
   open func palyEndHide(controlView: BMPlayerControlView, didPressButton button: UIButton){
      seek(0, completion: {
        self.play()
      })
      controlView.hidePlayToTheEndView()
